...
public class ClientControllerTest {
	...
	private static final String CLIENT_SEX = "Feminine";
	...
	public void shouldCreateValidClientTest() throws Exception {
		...
		Client savedClient = Client.builder().nome(CLIENT_NAME).build();
		...
		mockMvc.perform(request).andExpect(status().isCreated()).andExpect(jsonPath("id").isNotEmpty())
				.andExpect(jsonPath("name").value(dto.getNome()));
	...
	public void shouldGetClientById() throws Exception {
		...
		Client client = Client.builder().id(CLIENT_ID_1L).nome(CLIENT_NAME).build();
		...
		mockMvc.perform(MockMvcRequestBuilders.get(CLIENT_PATH.concat("/" + CLIENT_ID_1L)).accept(APPLICATION_JSON))
				...
				.andExpect(jsonPath("name").value(CLIENT_NAME)).andExpect(jsonPath("sex").value(CLIENT_SEX))
	...
	public void updateClientTest() throws Exception {
		...
		Client updating = Client.builder().id(CLIENT_ID_1L).nome(CLIENT_NAME).build();
		BDDMockito.given(service.getById(id)).willReturn(Optional.of(updating));
		Client updated = Client.builder().id(CLIENT_ID_1L).nome(CLIENT_NAME).build();
		...
		mockMvc.perform(request).andExpect(status().isOk()).andExpect(jsonPath("id").value(id))
				.andExpect(jsonPath("name").value(createNewClient().getNome()))
	...
	public void findClientsTest() throws Exception {
		...
		Client client = Client.builder().id(id).nome(CLIENT_NAME).build();

		BDDMockito.given(service.find(Mockito.any(Client.class), Mockito.any(Pageable.class)))
				.willReturn(new PageImpl<Client>(Arrays.asList(client), PageRequest.of(0, 100), 1));

		String queryString = String.format("?name=%s&cpf=%s&page=0&size=100", client.getNome());
	...
	private ClientDTO createNewClient() {
		return ClientDTO.builder().nome(CLIENT_NAME).id(CLIENT_ID_1L).build();
	}
